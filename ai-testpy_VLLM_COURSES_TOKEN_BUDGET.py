...
# domain_aliases: allow clients to refer to a domain by its last path component (e.g., 'ΨΥΧΑ1Υ').
# Populated at startup; only unique aliases are registered.
domain_aliases = {}

# domain_info: internal metadata about discovered lesson domains (path parts, folder path)
domain_info = {}
...
def query_model(prompt: str) -> str:
    """Call vLLM (OpenAI-compatible Chat Completions) with token budgeting."""
    ...
    # --- dynamic max_tokens ---
    prompt_tokens = approx_messages_tokens(messages)
    available = VLLM_MAX_CONTEXT - prompt_tokens - VLLM_SAFETY_MARGIN
    max_tokens = max(VLLM_MIN_COMPLETION, min(VLLM_MAX_COMPLETION, available))

    # If prompt alone is too large, trim QUOTES section (or tail) until it fits.
    if available < VLLM_MIN_COMPLETION:
        target_prompt_tokens = max(256, VLLM_MAX_CONTEXT - VLLM_SAFETY_MARGIN - VLLM_MIN_COMPLETION)
        user_text = messages[1]["content"]

        if "\nQUOTES:\n" in user_text:
            head, quotes_part = user_text.split("\nQUOTES:\n", 1)
            trimmed_quotes = truncate_text_to_tokens(
                quotes_part,
                max(128, target_prompt_tokens - approx_token_count(head) - 32),
            )
            user_text = head + "\nQUOTES:\n" + trimmed_quotes
        else:
            user_text = truncate_text_to_tokens(user_text, target_prompt_tokens)

        messages[1]["content"] = user_text

        prompt_tokens = approx_messages_tokens(messages)
        available = VLLM_MAX_CONTEXT - prompt_tokens - VLLM_SAFETY_MARGIN
        max_tokens = max(32, min(VLLM_MAX_COMPLETION, max(0, available)))
...
ΤΕΛΙΚΗ ΑΠΑΝΤΗΣΗ:"""
        ...
        # If prompts get too large (vLLM enforces strict context limits), trim QUOTES first.
        budget = VLLM_MAX_CONTEXT - VLLM_SAFETY_MARGIN - VLLM_MIN_COMPLETION
        if approx_token_count(final_prompt) > budget:
            q_lines = [ln for ln in quotes.splitlines()]
            while q_lines and approx_token_count(final_prompt) > budget:
                # remove one quote block (2 lines heuristic: quote line + blank / next)
                q_lines = q_lines[:-2] if len(q_lines) > 2 else []
                quotes = "\n".join(q_lines).strip()
                final_prompt = f"""{answer_system}
...
ΤΕΛΙΚΗ ΑΠΑΝΤΗΣΗ:"""
        ...
        try:
            ...
            lines_ = [ln.strip() for ln in quotes.splitlines() if ln.strip()]
...
def ask():
    ...
    # Resolve domain (supports full IDs, unique short aliases, and unique suffix matches).
    matched_domain = None
    for d in domain_data.keys():
        if d.lower() == str(domain_in).lower():
            matched_domain = d
            break
    if matched_domain is None:
        alias_key = str(domain_in).strip()
        if alias_key in domain_aliases:
            matched_domain = domain_aliases[alias_key]
    if matched_domain is None:
        suffix = "__" + str(domain_in).strip()
        candidates = [d for d in domain_data.keys() if d.endswith(suffix) or d == str(domain_in).strip()]
        if len(candidates) == 1:
            matched_domain = candidates[0]
    if matched_domain is None:
        matched_domain = str(domain_in)
...
def _dir_has_supported_files(folder: str) -> bool:
    try:
        for fn in os.listdir(folder):
            if fn.lower().endswith((".pdf", ".txt", ".md", ".docx", ".pptx")):
                return True
    except Exception:
        return False
    return False
...
def _discover_lesson_domains(base_folder: str):
    """Discover 'lesson' domains recursively.

    A lesson domain is defined as any directory that contains at least one supported file.
    Returns list of tuples: (domain_id, folder_path, rel_parts)
    """
    domains = []
    base_folder = os.path.abspath(base_folder)
    base_name = os.path.basename(base_folder.rstrip(os.sep)) or "BASE"
    for root, dirs, files in os.walk(base_folder):
        # Skip internal/cache dirs
        bn = os.path.basename(root)
        if bn.startswith(".") or bn in {"__pycache__", ".git"}:
            dirs[:] = []
            continue

        # If this directory contains supported files, treat it as a domain root and don't descend further.
        if any(f.lower().endswith((".pdf", ".txt", ".md", ".docx", ".pptx")) for f in files):
            rel = os.path.relpath(root, base_folder)
            rel_parts = [p for p in rel.split(os.sep) if p and p != "."]
            domain_id = base_name + "__" + "__".join(rel_parts) if rel_parts else base_name
            domains.append((domain_id, root, rel_parts))
            dirs[:] = []  # do not index nested subfolders as separate domains
            continue
    return domains


def _build_year_aggregates(discovered):
    """Build YEAR aggregates: YEAR__<Course>__<YEAR_n> from lesson domains."""
    # Group by (course, year)
    groups = {}
    for domain_id, folder, rel_parts in discovered:
        # expected rel_parts: [Course, YEAR_x, SEM_y, LessonDomain]
        if len(rel_parts) >= 2 and rel_parts[1].startswith("YEAR_"):
            course = rel_parts[0]
            year = rel_parts[1]
            key = (course, year)
            groups.setdefault(key, []).append(domain_id)

    built = 0
    for (course, year), members in sorted(groups.items()):
        agg_id = f"YEAR__{course}__{year}"
        # Merge vectors + chunks + meta from member lesson domains.
        all_chunks = []
        all_meta = []
        all_vecs = []
        ...
        for m in members:
            ...
            try:
                ...
                # If we cannot reconstruct (unexpected index type), skip aggregation for this member.
                pass

        if not all_vecs or not all_chunks:
            continue
        ...
        domain_data[agg_id] = {"chunks": all_chunks, "index": index, "meta": all_meta}
        built += 1
        print(f"🧩 YEAR index '{agg_id}' built from {len(members)} lesson domains.")

    return built


def load_domains():
    """Load (index) all discovered domains under BASE_FOLDER.

    Supports:
      - flat domains folder (legacy)
      - nested Courses/* structure (recursive discovery)
    """
    ...
    discovered = _discover_lesson_domains(BASE_FOLDER)

    if not discovered:
        print(f"⚠️ No domain subfolders found in {BASE_FOLDER}.")
        print("Expected either:")
        print(f"  {os.path.join(BASE_FOLDER, 'cybersecurity')}  (legacy flat mode)")
        print("or nested mode like:")
        print(f"  {os.path.join(BASE_FOLDER, 'Psychology', 'YEAR_1', 'SEM_1', 'ΨΥΧΑ1Υ')}")
        return

    # Build indexes for lesson domains
    for domain_id, folder, rel_parts in discovered:
        domain_info[domain_id] = {"folder": folder, "rel_parts": rel_parts}
        build_domain_index(domain_id, folder)

    # Build unique short aliases (leaf folder name)
    alias_candidates = {}
    for domain_id, _, rel_parts in discovered:
        if rel_parts:
            leaf = rel_parts[-1]
            alias_candidates.setdefault(leaf, set()).add(domain_id)
    domain_aliases.clear()
    for leaf, ids in alias_candidates.items():
        if len(ids) == 1:
            domain_aliases[leaf] = next(iter(ids))

    # YEAR aggregates
    _build_year_aggregates(discovered)
