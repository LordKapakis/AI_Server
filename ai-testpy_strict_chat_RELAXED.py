...
def sanitize_and_validate_quotes(quotes_text: str, retrieved_chunks: list, allowed_labels: list):
    """Keep only quotes that are grounded in the retrieved chunk they cite.

    Expected line format:
      - "…quote…" [n]

    Validation strategy (to avoid "always Δεν ξέρω"):
      1) Strict normalized substring match: _norm(quote) in _norm(chunk)
      2) Fallback word-overlap match if similarity is high enough (default 0.70)

    Returns:
      (clean_quotes_text, used_labels)
    """
    if not quotes_text:
        return "", []

    # Ensure we have int labels available
    try:
    ...
    OVERLAP_THRESHOLD = float(os.getenv("QUOTE_OVERLAP_THRESHOLD", "0.10"))
    ...
    for raw in quotes_text.splitlines():
        ...
        if lab not in label_set:
            continue
        if lab < 1 or lab > len(retrieved_chunks):
            continue

        hay = retrieved_chunks[lab - 1]
        hay = hay if isinstance(hay, str) else str(hay)

        # 1) Strict (but normalized) substring match
        ok = _norm(quoted) in _norm(hay)

        # 2) Fallback: accept if overlap is high (guards against small punctuation/newline differences)
        if not ok:
