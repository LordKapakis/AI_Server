...
def _norm(s: str) -> str:
    ...
    if not s:
        return ""
    # unify common smart punctuation
...
def auto_extract_quotes(question: str, retrieved_chunks: list, max_quotes: int = 4):
    ...
    This is a safety net when the model fails to output properly formatted quotes.
    Strategy:
      - split chunks into sentence-like spans
      - score by word overlap with the question
      - keep 8–20 word spans, verbatim
    Returns: list of tuples (quote_text, label_int)
    """
    qn = _norm(question)
    ...
    for i, ch in enumerate(retrieved_chunks, start=1):
        ...
        # split on sentence boundaries / new lines
        parts = re.split(r'(?<=[\.!\?;:])\s+|\n+', ch_raw)
        for part in parts:
            p = part.strip()
            if not p:
            ...
            wc = len(p.split())
            if wc < 8 or wc > 22:  # small buffer above 20
                continue
            pn = _norm(p)
            if not pn:
                continue
            ...
            if not pw:
                continue
            # overlap score: fraction of question words present in the span
            score = (len(pw & q_words) / max(1, len(q_words)))
            candidates.append((score, wc, p, i))

    # prefer higher score, then medium length
...
def messages_to_context_text(messages, max_messages=20, max_chars=6000):
    ...
    messages: list of dicts like {role: 'user'|'assistant'|'system', content: str}
    Keeps only the last max_messages items and truncates to max_chars characters.
    """
...
def sanitize_and_validate_quotes(quotes_text: str, retrieved_chunks: list, allowed_labels: list):
    """Validate model-produced quotes against retrieved chunks.

    Accepts several line formats, e.g.:
      - "quote text" [2]
      - “quote text” [2]
      - - quote text [2]
      - quote text [2]

    Then checks grounding using:
      1) normalized substring match
      2) fallback word overlap >= QUOTE_OVERLAP_THRESHOLD
    """
    if not quotes_text:
        return "", []

    try:
        label_set = set(int(x) for x in allowed_labels)
    except Exception:
        label_set = set()
    ...
    OVERLAP_THRESHOLD = float(os.getenv("QUOTE_OVERLAP_THRESHOLD", "0.70"))

    for raw in quotes_text.splitlines():
        line = raw.strip()
        if not line:
            continue

        # Extract label [n] first
        lm = re.search(r"\[(\d+)\]\s*$", line)
        if not lm:
            continue
        lab = int(lm.group(1))
        if lab not in label_set or lab < 1 or lab > len(retrieved_chunks):
            continue
        ...
        hay = retrieved_chunks[lab - 1]
        hay = hay if isinstance(hay, str) else str(hay)

        ok = _norm(quoted) in _norm(hay)
        if not ok:
            ok = word_overlap_ratio(quoted, hay) >= OVERLAP_THRESHOLD

        if ok:
            # enforce shorter quotes in output (best effort)
            words = quoted.split()
            if len(words) > 24:
                quoted = " ".join(words[:24])
            clean_lines.append(f'- "{quoted}" [{lab}]')
            if lab not in used_labels:
                used_labels.append(lab)

    return "\n".join(clean_lines).strip(), used_labels
...
def answer_question(domain, question, top_k=R_TOP_K, chat_messages=None):
    ...
    # Chat context (for conversational continuity)
    chat_context = messages_to_context_text(chat_messages or [])
    # In strict mode, retrieve a bit more to improve chance of grounded quotes
    strict_top_k = max(int(top_k), int(os.getenv('STRICT_TOP_K', '8'))) if STRICT_MODE else int(top_k)
    # Multi-query retrieval
    # In STRICT_MODE we avoid paraphrase drift; retrieve only with the user question.
    if STRICT_MODE:
        queries = [question]
    else:
    ...
    for q in queries:
        ...
        S, I = index.search(q_emb, strict_top_k)  # cosine similarity
...
Στόχος: Εξήγαγε 2–4 ΣΥΝΤΟΜΑ αποσπάσματα (8–20 λέξεις) που απαντούν άμεσα την ΕΡΩΤΗΣΗ.

Κανόνες:
- Η ΣΥΝΟΜΙΛΙΑ χρησιμοποιείται ΜΟΝΟ για να καταλάβεις σε τι αναφέρεται ο χρήστης (π.χ. \"αυτό\", \"εκείνο\").
- ΜΗΝ απαντήσεις σε προηγούμενες ερωτήσεις. Απάντησε ΑΥΣΤΗΡΑ μόνο στην τρέχουσα ΕΡΩΤΗΣΗ.
- ΜΗΝ χρησιμοποιείς τη ΣΥΝΟΜΙΛΙΑ ως πηγή facts. Facts μόνο από τις ΠΗΓΕΣ.
- ΜΟΝΟ άμεσες παραθέσεις (quotes) από τις ΠΗΓΕΣ. Καμία παράφραση εδώ.
- Κάθε quote να είναι 8–20 λέξεις (όσο πιο μικρό, τόσο καλύτερα).
...
ΣΥΝΟΜΙΛΙΑ (για συμφραζόμενα, όχι ως πηγή αλήθειας):
...
ΑΠΟΣΠΑΣΜΑΤΑ:"""
    ...
    if os.getenv('DEBUG_QUOTES','true').lower() in ('1','true','yes'):
        print('--- RAW QUOTES ---')
        print(quotes)


    # Validate extracted quotes against retrieved chunks to prevent hallucinated sources.
    quotes, quote_used_labels = sanitize_and_validate_quotes(quotes, retrieved, allowed_labels)
    if os.getenv('DEBUG_QUOTES','true').lower() in ('1','true','yes'):
        print('--- SANITIZED QUOTES ---')
        print(quotes)
        print('used_labels:', quote_used_labels)
    if STRICT_MODE and not quotes:
        # If we cannot produce verbatim, source-grounded quotes, we refuse to answer.
        return "Δεν ξέρω.", []
...
def ask():
    ...
    # ✅ NEW: chat history (OpenAI-style array: {role, content})
