...
# =========================
# Smart lexical sanity (prevents off-topic answers when retrieval is weak)
# =========================
# Minimal Greek stopwords (extend if needed)
_GREEK_STOPWORDS = {
    "και","ή","να","το","τα","τη","τι","τον","την","τους","τις","σε","στο","στη","στον","στην",
    "με","για","από","πως","πώς","που","πως","ότι","οτι","είναι","η","ο","οι","ένα","μια","μία",
    "ως","αν","θα","δεν","ναι","όχι","εγώ","εσύ","αυτό","αυτή","αυτά","αυτον","αυτην","αυτους","αυτες"
}
...
def _keyword_tokens(text: str):
    ...
    # keep greek/latin letters and digits
    t = re.sub(r"[^0-9a-zα-ωάέήίόύώϊϋΐΰ\s]", " ", t, flags=re.IGNORECASE)
    toks = [w for w in t.split() if len(w) >= 4 and w not in _GREEK_STOPWORDS]
...
def _lexical_sanity_pass(question: str, retrieved) -> bool:
    """Return True if retrieved text seems to actually mention the question keywords."""
    keys = _keyword_tokens(question)
    if not keys:
        return True  # nothing to check
    blob = " ".join([c for (c, _, _) in (retrieved or [])]).lower()
    hits = 0
    for k in keys[:8]:  # only check a few strongest keywords
        if k in blob:
            hits += 1
    # Require at least 1 keyword match to avoid totally off-topic answers
    return hits >= 1
...
def query_model(prompt: str) -> str:
    ...
    payload = {
        "model": VLLM_MODEL,
        "messages": [
            {"role": "system", "content": "Απάντα ΜΟΝΟ στα Ελληνικά. Απάντα κατευθείαν χωρίς πρόλογο/μετα-σχόλια. Χρησιμοποίησε ΜΟΝΟ τις πληροφορίες που σου δίνονται από τα αποσπάσματα. Αν δεν υπάρχουν αρκετά στοιχεία, απάντησε ΑΚΡΙΒΩΣ: \"Δεν υπάρχουν αρκετά στοιχεία στις πηγές.\""},
...
def answer_question(domain_id: str, question: str, chat_messages=None):
    ...
    # ✅ Lexical sanity: avoid off-topic answers when retrieval is weak
    if retrieved and (not _lexical_sanity_pass(question, retrieved)):
        retrieved = []
    if not retrieved:
        if STRICT_MODE:
            return (
                "Δεν υπάρχουν αρκετά στοιχεία στις πηγές.",
                [],
            )
        else:
            prompt = (
