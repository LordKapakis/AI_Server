...
"""
    ...
    # We DO NOT ask the LLM to create quotes (it tends to paraphrase).
    # Instead, we extract short verbatim spans directly from retrieved text.
    extracted = auto_extract_quotes(question, retrieved, max_quotes=4)

    if not extracted:
        # No grounded evidence -> refuse in strict mode, or give a generic fallback otherwise
        return ("Δεν ξέρω.", []) if STRICT_MODE else ("Δεν ξέρω.", [])

    quotes = "\n".join([f'- "{q}" [{lab}]' for (q, lab) in extracted])
    quote_used_labels = sorted({lab for (_, lab) in extracted})

    if os.getenv('DEBUG_QUOTES','false').lower() in ('1','true','yes'):
        print('--- AUTO QUOTES ---')
        print(quotes)
