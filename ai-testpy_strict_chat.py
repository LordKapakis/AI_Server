...
def sanitize_and_validate_quotes(quotes_text: str, retrieved_chunks: list, allowed_labels: list):
    """Validate that each extracted quote is a real verbatim substring of the cited chunk.

    Keeps only lines matching:
      - "..." [n]
    where n is an allowed label, and the quoted text appears verbatim in retrieved_chunks[n-1].
    ...
    for raw in str(quotes_text).splitlines():
        line = raw.strip()
        ...
        hay = retrieved_chunks[lab - 1] or ""
        if quoted and quoted in hay:
            clean_lines.append(f'- "{quoted}" [{lab}]')
            if lab not in used:
                used.append(lab)

    return "\n".join(clean_lines).strip(), used
...
Κανόνες (αυστηρά):
- Απάντησε ΜΟΝΟ στην τρέχουσα ΕΡΩΤΗΣΗ. Η ΣΥΝΟΜΙΛΙΑ είναι μόνο για συμφραζόμενα/αναφορές.
...
ΤΕΛΙΚΗ ΑΠΑΝΤΗΣΗ:"""
    ...
    # In STRICT_MODE, citations must match validated quote labels (prevents made-up [n]).
    if STRICT_MODE:
        if not used_labels:
            return "Δεν ξέρω.", []
        if 'quote_used_labels' in locals() and quote_used_labels:
            bad = [lab for lab in used_labels if lab not in quote_used_labels]
            if bad:
                return "Δεν ξέρω.", []
